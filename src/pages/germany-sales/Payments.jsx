...
export default function SalesPayments() {
  ...
  const columns = [
    ...
    { key: 'cashInBank', label: 'Cash in Bank', render: (val) => <span className="text-grey-70 font-medium">${val.toLocaleString()}</span> },
    ...
    return (
      <div>
        ...
        <div className="grid grid-cols-3 gap-6 mb-8">
          ...
          <div className="bg-white border border-grey-20 rounded-xl p-6 flex items-center gap-4">
            <div className="w-10 h-10 rounded-lg bg-green-light flex items-center justify-center"><Wallet className="w-5 h-5 text-green" /></div>
            <div>
              <p className="text-[11px] font-medium text-grey-40 uppercase">Cash in Bank</p>
              <p className="text-[22px] font-semibold text-grey-95">${selectedStudent.cashInBank.toLocaleString()}.00</p>
            </div>
          </div>
        </div>
  ...
  return (
    <div>
      <PageHeader greeting title="Payments" subtitle="Track student payments and cash in bank" />
