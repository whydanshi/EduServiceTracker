...
export default function ServiceMyTasks() {
  ...
  const taskLeads = useMemo(() => {
    let result = leads.filter(l =>
      ['Quality Check', 'Assigned', 'On Hold', 'In Review', 'Email Sent', 'QC Checked'].includes(l.serviceStatus)
    )
  ...
  const columns = [
    ...
    { key: 'salesPOC', label: 'Sales POC', render: (val) => <span className="text-grey-60">{val}</span> },
    {
      ...
      render: (val, row) => {
        if (['Quality Check', 'Assigned'].includes(row.serviceStatus)) {
          return <TaskButton label="Start QC Check" href={`/germany/service/lead/${row.id}`} />
        }
        if (['Email Sent', 'QC Checked'].includes(row.serviceStatus)) {
          return <TaskButton label="Send Email" href={`/germany/service/lead/${row.id}`} />
        }
        return <TaskButton label="Schedule Follow-up" href={`/germany/service/lead/${row.id}`} />
      },
    },
  ]
