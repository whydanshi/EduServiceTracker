...
const getNextTask = (lead) => {
  if (!lead.journey?.steps) return 'Start Journey'
  const currentStepIdx = lead.journey.currentStep ?? 0
  const currentStep = JOURNEY_STEPS[currentStepIdx]
  ...
  if (stepData?.status === 'completed') {
    ...
    return nextStep ? `Move to ${JOURNEY_STAGE_NAMES[nextStep.id] || nextStep.label}` : 'Journey Complete'
  }
  return `Complete ${JOURNEY_STAGE_NAMES[currentStep.id] || currentStep.label}`
}
...
export default function ServiceAllStudents() {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (val, row) => {
        // Calculate next due date (placeholder - would come from actual task scheduling)
      ...
      render: (val, row) => {
        ...
        return (
          <button
            onClick={(e) => {
              e.stopPropagation()
              navigate(`/germany/service/lead/${row.id}`)
            }}
  ...
  return (
    <div>
      <PageHeader title="Active Students" subtitle="Students in active service journey" />
      <div className="mb-5">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <button className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[12px] font-medium text-grey-60 border border-grey-20 hover:bg-grey-10">
      ...
      <QuickView
        ...
        onMarkStageComplete={(lead, remark) => {
          // Navigate to detail page for full stage management
          navigate(`/service/lead/${lead.id}`)
